...
export function renderVectorsPage(container) {
  ...
  const toolbarCtrl = renderToolbar(toolbar, TOOLS, (toolId) => {
    activeTool = toolId;
    updateScene();
    updatePanel();
  });

  // Canvas click handler for creating vectors
  canvasContainer.addEventListener('click', (e) => {
    ...
    const name = String.fromCharCode(65 + vectors.length); // A, B, C...
    vectors.push({
  ...
  function updateScene() {
    ...
    if (activeTool === 'sum' && vectors.length >= 2) {
      const vecs = vectors.map((v) => ({ x: v.x, y: v.y }));
      const sum = addVectors(...vecs);
    ...
    if (activeTool === 'decompose' && selectedIndex >= 0 && selectedIndex < vectors.length) {
      const v = vectors[selectedIndex];
      // Components are already drawn if visibility is on, but force-draw them here
      if (!vis.components) {
        const xArrow = createArrow(
          { x: v.originX, y: v.originY },
          { x: v.x, y: 0 },
  ...
  function updatePanel() {
    ...
    // Selected vector properties
    if (selectedIndex >= 0 && selectedIndex < vectors.length) {
      const v = vectors[selectedIndex];
      const mag = magnitude({ x: v.x, y: v.y });
      const dir = direction({ x: v.x, y: v.y });

      sections.push({
        title: `Vettore ${v.name}`,
        content:
          createInputRow('X', 'vec-x', v.x, '', 'step="0.1"') +
          createInputRow('Y', 'vec-y', v.y, '', 'step="0.1"') +
    ...
    if (activeTool === 'difference' && vectors.length >= 2) {
      ...
      sections.push({
        ...
        content:
          ...
          createPropertyRow('|D|', mag.toFixed(2)) +
          createPropertyRow('\u03b8', dir.toFixed(1) + '\u00b0'),
      });
    }
    ...
    if (activeTool === 'decompose' && selectedIndex >= 0 && selectedIndex < vectors.length) {
      ...
      sections.push({
        ...
        content:
          ...
          createPropertyRow('|' + v.name + '|', mag.toFixed(2)) +
          createPropertyRow('\u03b8', dir.toFixed(1) + '\u00b0'),
      });
    }
    ...
    // Visibility menu
    const unsubVis = renderVisibilityMenu(rightPanel);
  ...
  function wireUpEvents() {
    const xInput = rightPanel.querySelector('#vec-x');
    const yInput = rightPanel.querySelector('#vec-y');
    ...
    // Vector list click
    const vecRows = rightPanel.querySelectorAll('[data-vec-index]');
    vecRows.forEach((row) => {
  ...
  // Subscribe to state changes (visibility toggles, theme)
